  ...
  [
    {
      ...
      large_number_str: "98765432109876543210",
...
const complexSchema: OutputBlockVariables[] = [
  {
    name: "arr",
    type: "Object",
    struct: [
        ...
        struct: [
          ...
          },
          {
            ...
            struct: [
              ...
              { name: "notifications", type: "StringArray" },
              { name: "score_threshold", type: "Double" },
              { name: "send_summary", type: "Boolean" },
            ],
          },
          { name: "last_coordinates", type: "DoubleArray" },
        ],
      },
      { name: "transaction_ids", type: "LongArray" },
      { name: "event_timestamps", type: "DateTimeArray" },
      { name: "large_number_str", type: "BigInteger" },
      { name: "precise_value_str", type: "BigDecimal" },
      {
        name: "metadata",
        type: "Object",
        struct: [
          ...
          { name: "processing_flags", type: "BooleanArray" },
          {
            name: "nested_array",
            type: "ObjectArray",
            struct: [
              { name: "history", type: "StringArray" },
              { name: "timestamp", type: "DateTime" },
            ],
          },
          { name: "file_ref", type: "File" },
        ],
      },
      { name: "optional_notes", type: "String" },
      {
        name: "maybe_null_object",
        type: "Object",
        struct: [{ name: "config_value", type: "Double" }],
      },
    ],
  },
];
...
describe("OutputValidator", () => {
  test("Сложная структура", () => {
    const outputValidator = new OutputValidator();

    expect(outputValidator.validateOutput(complexData, complexSchema)).toEqual(true);
  });
});
